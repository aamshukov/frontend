...
bool logger::initialize(const string_type& file_name, operation_status& status)
{
    bool result = false;
    ...
    try
    {
        ...
        {
            OPERATION_FAILED(status::custom_code::error, L"Failed to initialize logger: stream is either open or in a bad condition.")
        }
    ...
    {
        OPERATION_FAILED(status::custom_code::error, L"Failed to initialize logger: error occurred.")
    }

    return result;
}

bool logger::uninitialize(operation_status& status)
{
    bool result = false;
    ...
    {
        OPERATION_FAILED(status::custom_code::error, L"Failed to uninitialize logger: error occurred.")
    }
...
void logger::format_time(char_type* buffer, uint16_t buffer_size)
{
    ...
    snwprintf(buffer, buffer_size, TRUNCATE, L"%02d%02d%04d %02d:%02d:%02d", new_time.tm_mday, new_time.tm_mon+1, new_time.tm_year+1900, new_time.tm_hour, new_time.tm_min, new_time.tm_sec);
}
...
void logger::log(const char_type* file_name, uint32_t line, const char_type* function_name, severity level, const char_type* format_template, ...)
{
    ...
    if(my_stream.is_open() && !(my_stream.bad() || my_stream.fail()))
    {
        if(level <= my_severity)
        {
            ...
            snwprintf(buffer, array_size(buffer), TRUNCATE, L"[%s]:[%s(%ld):%s][%s]  ", time_buffer, file_name, line, function_name, severity_name);
